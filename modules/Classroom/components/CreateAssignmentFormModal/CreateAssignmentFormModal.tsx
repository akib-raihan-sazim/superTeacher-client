...
const AssignmentFormModal: React.FC<IAssignmentFormModalProps> = ({
  opened,
  onClose,
  classroomId,
  assignment,
  ...
  const {
    ...
    resolver: zodResolver(assignment ? assignmentFormSchemaEdit : assignmentFormSchema),
    mode: "onBlur",
  });
  ...
  const onSubmit = async (data: TAssignmentFormValues) => {
    ...
    try {
      ...
      showNotification({
        title: assignment ? "Update failed" : "Creation failed",
        message: assignment ? "Failed to update assignment" : "Failed to create assignment",
        color: "red",
      });
    } finally {
      setIsLoading(false);
